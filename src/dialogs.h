...
public:
    ...
    // Message dialog
    void messageDialog(const QString &title, const QString &message)
    {
        emit messageStarted(title, message);
    }

    // Open URL dialog
    void openUrlDialog(const QUrl &url)
    {
        emit openUrlStarted(url);
    }

    // Selection dialog
    void selectionDialog(const QString &title, const QStringList &items, std::function<void(int, bool)> callback, const QString &checkboxText);
