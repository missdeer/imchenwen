...
DownloaderItem::DownloaderItem(
    const QString &filepath, const QList<QUrl> &urls, const QUrl &danmkauUrl, const QUrl &subtitleUrl, bool isDash, QObject *parent)
    : DownloaderAbstractItem(filepath, danmkauUrl, subtitleUrl, parent), m_process(nullptr), m_finished(0), m_total(urls.length()), m_isDash(isDash)
{
    // Create tempdir
