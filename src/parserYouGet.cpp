...
ParserYouGet ParserYouGet::s_instance;

ParserYouGet::ParserYouGet(QObject *parent) : ParserBase(parent)
{
    // Connect
    connect(&m_process, QOverload<int, QProcess::ExitStatus>::of(&QProcess::finished), this, &ParserYouGet::parseOutput);
    connect(&m_process, &QProcess::errorOccurred, [&]() { showErrorDialog(m_process.errorString()); });
}

ParserYouGet::~ParserYouGet()
{
    // Kill you-get process
...
void ParserYouGet::runParser(const QUrl &url)
{
    ...
    m_process.start(userResourcesPath() + QStringLiteral("/you-get"), args, QProcess::ReadOnly);
}

void ParserYouGet::parseOutput()
{
...
void ParserYouGet::parseEpisode(QJsonObject episode)
{
    // Check error
    if (!episode[QStringLiteral("err")].isNull())
    {
        showErrorDialog(QStringLiteral("You-Get Error: ") + episode[QStringLiteral("err")].toString());
        return;
    }
