...
void PlayDialog::setMediaInfo(const QString &originalUrl, MediaInfoPtr mi)
{
    ...
    {
        if (!ui->listSubtitle->count() && mi->subtitles.isEmpty())
        {
            ui->labelSubtitle->setVisible(false);
            ui->listSubtitle->setVisible(false);
        }
    }
    else
    {
        if (!mi->subtitles.isEmpty())
        {
            ...
            ui->listSubtitle->setVisible(true);
        }
    }

    for (auto subtitle : mi->subtitles)
    {
        QListWidgetItem *item = new QListWidgetItem(subtitle->manual ? QIcon(":/manualgenerated.png") : QIcon(":/autogenerated.png"), subtitle->language, ui->listSubtitle);
        ...
        item->setFont(font);
        item->setBackgroundColor(ui->listMedia->count() % 2 ? Qt::white : QColor(0xf0f0f0));
...
void PlayDialog::doOk()
{
    ...
    {
        currentRow = ui->listSubtitle->currentRow();
